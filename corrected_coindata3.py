...
combined_data = eth_data.join(btc_data, lsuffix='_eth', rsuffix='_btc', how='inner')
...
combined_data['eth_rsi'] = 100 - (100 / (1 + combined_data['eth_return'].rolling(14).apply(lambda x: (x[x > 0].sum() / -x[x < 0].sum()) if x[x < 0].sum() != 0 else np.nan, raw=True)))
combined_data['btc_rsi'] = 100 - (100 / (1 + combined_data['btc_return'].rolling(14).apply(lambda x: (x[x > 0].sum() / -x[x < 0].sum()) if x[x < 0].sum() != 0 else np.nan, raw=True)))
